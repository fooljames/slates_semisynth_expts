...
if __name__ == "__main__":
    ...
    filenames = glob.glob(os.path.join(Settings.DATA_DIR, 'MSLR/Fold1', '*.txt'), recursive=True)
    with open(os.path.join(Settings.DATA_DIR, 'MSLR/Fold1/mslr.txt'), 'w') as outfile:
    ...
    for foldID in range(1, 6):
        for fraction in ['train', 'vali', 'test']:
            mslrData = Datasets()
            mslrData.loadTxt(Settings.DATA_DIR + 'MSLR-WEB10K/Fold' + str(foldID) + '/' + fraction + '.txt',
                             'MSLR10k-' + str(foldID) + '-' + fraction)
            del mslrData
    ...
    for foldID in range(1, 6):
        for fraction in ['train', 'vali', 'test']:
            mslrData = Datasets()
            mslrData.loadTxt(Settings.DATA_DIR + 'MSLR/Fold' + str(foldID) + '/' + fraction + '.txt',
                             'MSLR-' + str(foldID) + '-' + fraction)
            del mslrData
